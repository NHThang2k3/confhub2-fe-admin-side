...
// Định nghĩa các hằng số để dễ quản lý và nhất quán với hook
...
interface ConfigurationStepProps {
  ...
  chunkDelay: number; // <<< PROP MỚI
  setChunkDelay: (delay: number) => void; // <<< PROP MỚI
...
const ConfigurationStep: React.FC<ConfigurationStepProps> = ({
  ...
  chunkSize,
  setChunkSize,
  chunkDelay, // <<< NHẬN PROP MỚI
  setChunkDelay, // <<< NHẬN PROP MỚI
  apiModels,
  setApiModel,
  apiStepsForUploader,
  isCrawling,
          ...
          <div className='rounded-md border border-gray-200 bg-gray-5 p-5 space-y-4 shadow-sm h-full'>
            ...
            <div className='flex items-center mb-2'>
              <input
                ...
                disabled={isCrawling}
              />
              <label htmlFor='enable-chunking' className={`ml-2 text-sm font-medium text-gray-900 ${isCrawling ? 'opacity-60' : 'cursor-pointer'}`}>
                {t('chunking.enableCheckbox')}
              </label>
            </div>
            <p className="text-xs text-gray-500 mb-4">{t('chunking.enableDescription')}</p>

            {/* Container cho các cài đặt phụ thuộc vào chunking */}
            <div className={`space-y-4 transition-opacity duration-300 ${!enableChunking ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              {/* --- SECTION MỚI: CHUNK DELAY CONFIGURATION --- */}
              <div>
                <label htmlFor='chunk-delay' className='block text-sm font-medium text-gray-700'>
                  {t('chunking.delayLabel', { min: MIN_CHUNK_DELAY, max: MAX_CHUNK_DELAY })}
                </label>
                <div className="mt-1">
                  <input
                    id='chunk-delay'
                    ...
                    onChange={e => {
                      ...
                      // Chỉ gọi setChunkDelay nếu giá trị là một số
                      if (!isNaN(value)) {
                        setChunkDelay(value);
                      }
                    }}
